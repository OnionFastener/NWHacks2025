...
drug_pages = soup.find('div', class_ = 'ddc-grid-col-6 col-list-az').find_all('ul')

def get_drugs(page):
    # Locate the drug entries on the page
    drugs_list = page.find_all('ul', class_='ddc-list-column-2')
    ...
    for drugs in drugs_list:
        for drug in drugs.find_all('li'):
            ...
            if (common_name.lower() in common_names):
                drugs_data.append({
...
for pages in drug_pages:
    for page in pages.find_all('li'):
        link = page.find('a')['href']
        page_url = urljoin("https://www.drugs.com", link)
...
df.to_csv('drugs_data3.csv', index=False)

print("Data scraping complete. Saved to drugs_data.csv.")
